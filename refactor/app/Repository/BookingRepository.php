...
class BookingRepository extends BaseRepository
{
    ...
    public function jobEnd($post_data = array())
    {
        ...
        TeHelper::sendMail($email, $name, $subject,$data);
    ...
    public function sendPushNotificationToSpecificUsers($users, $job_id, $data, $msg_text, $is_need_delay)
    {
        ...
        if ($data['notification_type'] == 'suitable_job' && $data['immediate'] == 'no') {
    ...
    public function getPotentialTranslators(Job $job)
    {
        ...
        if ($job->certified) {
            switch($job->certified){
                ...
                default:
                    $translator_level = [
                        'Certified','Certified with specialisation in law','Certified with specialisation in health care','Layman', 'Read Translation courses'
                    ];
                    break;
            } 
        }
    ...
    private function changeTimedoutStatus($job, $data, $changedTranslator)
    {
        ...
        TeHelper::sendMail($email, $name, $subject,$dataEmail);
        return true;
    }
    ...
    private function changeStartedStatus($job, $data)
    {
        ...
        if ($data['status'] == 'completed') {
            ...
            $user_translator = $job->translatorJobRel->where('completed_at', Null)->where('cancel_at', Null)->first();

            $email = $user_translator->user->email;
            $name = $user_translator->user->name;
    ...
    private function changePendingStatus($job, $data, $changedTranslator)
    {
        ...
        $mail_syntax = 'emails.status-changed-from-pending-or-assigned-customer';
        ...
        if ($data['status'] == 'assigned' && $changedTranslator) {
            ...
            $subject = 'Bekräftelse - tolk har accepterat er bokning (bokning # ' . $job->id . ')';
            TeHelper::sendMail($email, $name, $subject,$dataEmail,'emails.job-accepted');

            $translator = Job::getJobsAssignedTranslatorDetail($job);
            $mail_syntax = 'emails.job-changed-translator-new-translator';
        ...
        $job->save();
        TeHelper::sendMail($email, $name, $subject,$dataEmail,$mail_syntax);
        return true;
    ...
    private function changeAssignedStatus($job, $data)
    {
        ...
        if (in_array($data['status'], ['withdrawbefore24', 'withdrawafter24', 'timedout'])) {
            ...
            $job->admin_comments = $data['admin_comments'];
                ...
                if ($job->user_email) $email = $job->user_email;

                $subject = 'Information om avslutad tolkning för bokningsnummer #' . $job->id;
                TeHelper::sendMail($email, $name, $subject,$dataEmail,'emails.status-changed-from-pending-or-assigned-customer');
                ...
                TeHelper::sendMail($email, $name, $subject,$dataEmail,'emails.job-cancel-translator');
            $job->save();
            return true;
        }
        return false;
    ...
    public function sendChangedTranslatorNotification($job, $current_translator, $new_translator)
    {
        ...
        TeHelper::sendMail($email, $name, $subject,$data,'emails.job-changed-translator-customer');
        ...
        if ($current_translator) {
            ...
            $email = $user->email;
            $data['user'] = $user;
            TeHelper::sendMail($email, $name, $subject,$data,'emails.job-changed-translator-old-translator');
        }
        ...
        $email = $user->email;
        $data['user'] = $user;
        TeHelper::sendMail($email, $name, $subject,$data,'emails.job-changed-translator-new-translator');
    ...
    public function acceptJob($data, $user)
    {
        ...
        if (!Job::isTranslatorAlreadyBooked($data['job_id'], $cuser->id, $job->due)) {
            if ($job->status == 'pending' && Job::insertTranslatorJobRel($cuser->id, $data['job_id'])) {
                ...
                TeHelper::sendMail($email, $name, $subject,$data,'emails.job-accepted');
    ...
    public function getAll(Request $request,$filter, $limit = null)
    {
        $requestdata = $request->all();
        $cuser = $request->__authenticatedUser;
        $consumer_type = $cuser->consumer_type;
        $allJobs = Job::query()->filter($filter);
